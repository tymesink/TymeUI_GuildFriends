...
local ldb_name_guild = '|cFFFFB366TymeUI|r-Guild';
local ldb_name_friends = '|cFFFFB366TymeUI|r-Friends';

local f = CreateFrame( "Frame", "TymeUIGuildFriends", UIParent)
if wowTOC >= 20501 then
...
local clientsTable = {
	...
	["WLBY"] = 17,--Crash 4
	["OSI"] = 18,--Diablo 2
	["FORE"] = 19,--COD: Vanguard
	["ANBS"] = 20,--Diablo Immortal
	["GRY"] = 21,--Arclight Rumble
}
...
local function SetToastData( index, inGroup )
	...
	elseif client == 5 or client == 8 then--Battle.net Desktop
		...
		elseif client == 19 then
			toast.class:SetTexture"Interface\\FriendsFrame\\Battlenet-CallOfDutyVanguardicon"
		elseif client == 20 then
			toast.class:SetTexture"Interface\\FriendsFrame\\Battlenet-DiabloImmortalicon"
		elseif client == 21 then
			toast.class:SetTexture"Interface\\FriendsFrame\\Battlenet-WarcraftArclightRumbleicon"
		end
		toast.class:SetTexCoord( .2, .8, .2, .8 )
		toast.name:SetTextColor( .8, .8, .8 )
		toast.faction:SetTexture""
...
function f:SetupConfigMenu()
	configMenu = CreateFrame("Frame", "TymeUIGuildFriendsConfigMenu")
	...
	StaticPopupDialogs.SET_ABGF_SCALE = {
		...
		hasEditBox = 1,
		maxLetters = 4,
		OnAccept = TymeUIGuildFriends.SetCustomScale,
		OnShow = function(self) CloseDropDownMenus() self.editBox:SetText(config.scale*100) self.editBox:SetFocus() end,
		OnHide = ChatEdit_FocusActiveWindow,
		EditBoxOnEnterPressed = function(self) local p=self:GetParent() TymeUIGuildFriends.SetCustomScale(p) p:Hide() end,
	...
	options = {
		{ text = ("|cffffb366TymeUI|r Guild & Friends (%s)"):format( GetAddOnMetadata( addonName, "Version" ) ), isTitle = true },
...
f.GuildBlock = ldb:NewDataObject(ldb_name_guild, {
	type = "data source",
	text = GUILD,
	icon = [[Interface\AddOns\TymeUI_GuildFriends\guild]],
...
f.FriendsBlock = ldb:NewDataObject(ldb_name_friends, {
	type = "data source",
	text = FRIENDS,
	icon = [[Interface\AddOns\TymeUI_GuildFriends\friends]],
	OnEnter = function(self)
		isGuild = false
		ShowFriends()
		AnchorTablet(self)
...
function f:ADDON_LOADED( addon )
	...
	GuildFriendsDB = GuildFriendsDB or defaultConfig
	config = GuildFriendsDB
